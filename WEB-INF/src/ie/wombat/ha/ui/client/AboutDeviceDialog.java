 ...
 */
public class AboutDeviceDialog extends DialogBox {

  interface Binder extends UiBinder<Widget, AboutDeviceDialog> { }
  ...
  public AboutDeviceDialog(Long deviceId) {
	  
	  this.deviceId = deviceId;
	  
    //setText("About ZigBee Console App");
    ...
    getDeviceInfo();
    getBasicAttribute (deviceId, 0x0000, zclVersionEl);
	getBasicAttribute (deviceId, 0x0001, appVersionEl);
	getBasicAttribute (deviceId, 0x0002, stackVersionEl);
	getBasicAttribute (deviceId, 0x0003, hwVersionEl);
	
	getBasicAttribute (deviceId, 0x0004, manufacturerEl);
	getBasicAttribute (deviceId, 0x0005, modelIdEl);
	getBasicAttribute (deviceId, 0x0006, dateEl);
	getBasicAttribute (deviceId, 0x0007, powerEl);
  }
  ...
  private void getDeviceInfo () {
		devicesService.getDeviceInfo(deviceId, new AsyncCallback<DeviceInfo>() {
			...
			public void onSuccess(DeviceInfo result) {
				setText("Device View. " + result.name + " " + result.addr64.toString());
			}
  ...
  private void getBasicAttribute (final Long deviceId, final int attrId, final SpanElement el) {
	  int[] attrIds = {attrId};
		zigbeeService.getAttributeValues(deviceId, 1,  0x104, 0x0, attrIds, 
